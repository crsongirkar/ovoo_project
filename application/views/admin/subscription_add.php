...
<input type="hidden" name="currency" value="<?php echo ovoo_config('currency'); ?>">
<input type="hidden" name="currency_symbol" value="<?php echo ovoo_config('currency_symbol'); ?>">

<div class="form-group row">
  <div class="col-sm-offset-3 col-sm-9 m-t-15">
    <button type="submit" class="btn btn-sm btn-primary waves-effect">
      <span class="btn-label"><i class="fa fa-plus"></i></span><?php echo trans("add"); ?>
    </button>
    <button type="button" class="btn btn-sm btn-white m-l-5 waves-effect" data-dismiss="modal"><?php echo trans("close"); ?></button>
  </div>
</div>
</form>
...
jQuery(document).ready(function() {
    ...
    // Format amount field with currency symbol
    $('input[name="paid_amount"]').on('blur', function() {
        var symbol = '<?php echo ovoo_config("currency_symbol"); ?>';
        var val = $(this).val().replace(symbol, '').trim();
        if(val && !isNaN(val)) {
            $(this).val(symbol + ' ' + parseFloat(val).toFixed(2));
        }
    });
});
</script>